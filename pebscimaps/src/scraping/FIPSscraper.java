...
public class FIPSscraper {

    static String FIPS = "01000        Alabama\n" + //
    ...
    public static void main(String[] args){
        ...
        String[] fipsCodes = FIPS.split("\n");

        StringBuilder jsonBuilder = new StringBuilder();

        for(int i = 0; i < fipsCodes.length; i++){
            String[] fipsElements = fipsCodes[i].split("        ");
            String FIPScode = fipsElements[0];
            String County = fipsElements[1];

            jsonBuilder.append("    \"").append(FIPScode.strip()).append("\": {\n")
                        .append("        \"name\": \"").append(County).append("\"\n")
                        .append("    },\n");
        }
        jsonBuilder.setLength(jsonBuilder.length() - 2); // Remove last comma and newline


        try (FileWriter file = new FileWriter(filePath, true)) { // Append mode enabled
            file.write(jsonBuilder.toString());
            System.out.println("Successfully wrote JSON-like data to " + filePath);
        } catch (IOException e) {
